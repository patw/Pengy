...
def _read_file(path: str, offset: int | None = None,
    ...
    try:
        ...
        # A hard-invalid byte sequence already raised UnicodeDecodeError above;
        # this catches the other half of the binary problem — bytes that decode
        # as valid UTF-8 but aren't text (a UTF-16 file leaves a NUL between
        # every ASCII byte).
        if _looks_binary(text)[0]:
            return f"Error: File appears to be binary (not text): {path}"
...
class _AskpassHelper:
    ...
    leak back into tool output and the chat log. Directory/script/password file
    are all single-use and removed as soon as the command exits.
    ...
    def __init__(self, password: str):
        ...
        # Separate 0600 password file; the script just cats it. The path is
        # generated by us (no user input), so quoting it is always safe.
        self._pw_path = os.path.join(self._dir, "pw")
        with open(self._pw_path, "w") as fh:
            fh.write(password)
        os.chmod(self._pw_path, 0o600)
        self.path = os.path.join(self._dir, "askpass.sh")
        with open(self.path, "w") as fh:
            fh.write("#!/bin/sh\ncat '{}'\n".format(self._pw_path))
        os.chmod(self.path, 0o700)

    def cleanup(self):
        for p in (self.path, self._pw_path):
            try:
                os.unlink(p)
            except OSError:
                pass
