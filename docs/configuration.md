...
Pengy stores its settings in `~/.config/pengy/settings.json`. This file is shared across all three interfaces (GUI, CLI, Web) and all three editions (Python, Rust, C++).
...
| `api_key` | string | `""` | API key (stored in plaintext — protect your config dir). A local server needs none |
...
| `max_tool_calls_per_turn` | int | `25` | Max tool calls the LLM can make in a single turn before Pengy forces a response. |
